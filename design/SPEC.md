...
scene / choice ノードに任意フィールド `diagram` を追加できる:

```json
{
  "type": "choice",
  "prompt": "このER図の問題点はどこ?",
  "diagram": { "src": "diagrams/s2e1-er.svg", "caption": "現行システムのER図", "alt": "顧客・注文・商品の3テーブル構成" },
  "options": [ ... ]
}
```

- `src` は data/ からの相対パス。SVGファイルは `data/diagrams/` に置く。
