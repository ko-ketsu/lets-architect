...
- シーズン3準備: ドキュメント整理と smoke-test 強化は完了。**design/SEASON3.md ドラフトをレビュー中**(T21、レビュー2巡目反映済み: 執筆原則A〜C 追加・s3e3 減量)。骨子: S3は「選ぶ力」— 障害耐性を見据えた技術選定(ネットワーク構成/VM・コンテナ・サーバーレス/SQL・NoSQL の3話)。障害対応ライブ形式は不採用(選定の根拠として障害シナリオを想定させる形に変更)。エンジン変更なしの方針
