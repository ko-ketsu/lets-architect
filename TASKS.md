...
- **フェーズ6実装中**(2026-07-05 着手): T22 シナリオ+図解 / T23 立ち絵を Sonnet エージェント2本に委譲して並行実行中。完了後 Fable が T24 統合・検証(顧客キャラ確定済み: 綾瀬玲=ayase.svg / 熊谷剛=kumagai.svg / 氷室悟=himuro.svg)
...
- [ ] T22: シーズン3エピソード3本+図解SVG(Sonnet)
  - s3e1 ネットワーク構成 / s3e2 実行基盤 / s3e3 データ層の冗長構成。SEASON3.md と SPEC 10.1・10.2 を遵守、お手本は s2e1
  - 完成条件: smoke-test 全パス(first/last が S 非到達含む)+ check-routes 全話 OK(S 1本・280〜290)
- [ ] T23: 顧客キャラ立ち絵3体(Sonnet)— SPEC 9.2 遵守、スクリーンショット目視確認してからコミット
- [ ] T24: 統合・検証(Fable)— index.json 追加(difficulty: 3)/ 出題図のネタバレ点検 / Playwright 実プレイ目視
- [ ] T25: push → 本番公開(ユーザー)
