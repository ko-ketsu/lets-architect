...
### 将来(初期リリース後)
- [ ] **シーズン2: 図解出題の実装**(ユーザー決定済み 2026-07-02)
  - 方式確定: 手作りSVG(data/diagrams/)+「図を読んで選択肢で答える」形式。Mermaid同梱・タップ間違い探し・D&Dは不採用
  - 仕様は design/SPEC.md 第8章に記載済み(diagram フィールド拡張・SVG制作ガイドライン)
  - 実装内容: エンジンの diagram 対応 / UI(図表示+タップ拡大モーダル)/ smoke-test に SVG 存在チェック追加 / 図入りエピソード
  - シーズン2テーマ「形にする」(アプリ設計・API設計)は ER図・クラス図・シーケンス図と相性が良い
- [ ] シーズン2〜4 のエピソード追加
- [ ] 用語集の独立ページ
- [ ] バッジ・実績システム
